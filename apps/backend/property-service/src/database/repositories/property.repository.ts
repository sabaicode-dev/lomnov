...
export class PropertyRepository {
  public async create(
    ...
    try {
      ...
      const imageUrls = await Promise.all(files.images.map(file => uploadFileToS3Service.uploadFile(file)));
    ...
    try {
      ...
      if (files.images && files.images.length > 0) {
        if (existingProperty.images && existingProperty.images.length > 0) {
          await Promise.all(existingProperty.images.map(image => deleteFileFromS3Service.deleteFile(image)));
        }
        propertyData.images = await Promise.all(files.images.map(file => uploadFileToS3Service.uploadFile(file)));
      }
  ...
  public async delete(propertyId: string): Promise<boolean> {
    try {
      ...
      if (property.images && property.images.length > 0) {
        await Promise.all(property.images.map(image => deleteFileFromS3Service.deleteFile(image)));
      }
