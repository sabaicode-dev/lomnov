...
class SocialAuthService {
  ...
  public async getOAuthToken(
    ...
    try {
      ...
      const response = await axios.post(cognitoDomain, params.toString(), {
        headers,
      });
      ...
      if (existingUser && existingUser.UserStatus !== "EXTERNAL_PROVIDER") {
        ...
        try {
          ...
          const res = await this.updateUserCongitoAttributes(
          ...
          if (axios.isAxiosError(error) && error.response?.status === 400) {
            ...
            const existingUserResponse = await axios.get(
              `${configs.userServiceUrl}/api/v1/users?userName=${userInfo.sub}`
            );
      ...
      const groupExist = await this.checkUserInGroup(
        existingUser?.Username!,
        state!
      );
      if (!groupExist) {
  ...
  async checkUserInGroup(
    ...
    try {
      ...
      const filterGroupName =
        response.Groups?.some((group) => group.GroupName === groupName) ||
        false;
      console.log("filterGroupName::: ", filterGroupName);

      return (
        response.Groups?.some((group) => group.GroupName === groupName) || false
      );
    } catch (error) {
      //@ts-ignore
  ...
  async createNewUser(userInfo: any, groupName: string): Promise<string> {
    try {
      ...
      console.log(
        `new user loging with google::: google_${userInfo.identities[0].userId}`
      );
      await this.updateUserCongitoAttributes(
        `google_${userInfo.identities[0].userId}`,
        { "custom:roles": groupName! }
      );
      return user.data._id;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response?.status === 400) {
        const existingUserResponse = await axios.get(
          `${configs.userServiceUrl}/api/v1/users?userName=${userInfo.name}`
        );
        console.log(existingUserResponse.data);
