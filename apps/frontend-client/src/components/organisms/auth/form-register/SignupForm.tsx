...
const signupSchema = z.object({
  ...
  username: z
    .string()
    .min(1, "Username is required")
    .regex(/^(?!\d+$)[\w\s]+$/, "Username cannot be only numbers")
    .refine(
...
const SignupForm: React.FC = () => {
  ...
  const onSubmit = async (data: SignupData) => {
    ...
    try {
      const response = await axiosInstance.post(`${API_ENDPOINTS.SIGN_UP}`, data);
        ...
        <fieldset>
          <InputField
            ...
            register={{
              ...
              onKeyDown: (e: any) => handleKeyDown(e, usernameInputRef),
            }}
            error={emailErrorMessage || errors.email?.message}
            isFocused={emailFocused}
            setIsFocused={setEmailFocused}
          ...
          <InputField
            ...
            register={{
              ...
              onKeyDown: (e: any) => handleKeyDown(e, passwordInputRef),
            }}
            error={usernameErrorMessage || errors.username?.message}
            isFocused={userNameFocused}
            setIsFocused={setUserNameFocused}
