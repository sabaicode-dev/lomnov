        ...
        if (liveSearch.trim() === "") {
            ...
            setSearchState(() => {
                return agents.filter(item => {
                    const title = item.userName[0]?.toLowerCase();
                    const category = item.address[0]?.toLowerCase();
                    const price = item?.email.toString();
                    const contact = item?.phoneNumber.toString();
                    ...
                    return (title && title.includes(liveSearch.toLowerCase())) ||
                        ...
                        || (price && price.includes(liveSearch)) || (contact && contact.includes(liveSearch.toLowerCase()));
                });
            });
        }
    }, [liveSearch, agents]); // Re-run the effect when liveSearch or properties change
