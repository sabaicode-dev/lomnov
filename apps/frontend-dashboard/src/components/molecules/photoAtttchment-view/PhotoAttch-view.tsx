"use client";
import React, { useState, ChangeEvent } from "react";
import { TiDeleteOutline } from "react-icons/ti";

interface PhotoAttachmentProps {
  initialImages?: string[]; // Accept images from parent
}

const PhotoAttachment = ({ initialImages = [] }: PhotoAttachmentProps) => {
  const [images, setImages] = useState<string[]>(initialImages);

  // Add new images from file input
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { files } = e.target;
    if (files && files.length > 0) {
      const newImages = Array.from(files).map((file) =>
        URL.createObjectURL(file)
      );
      setImages((prevImages) => [...prevImages, ...newImages]);
    }
  };

  // Remove an image

  return (
    <div className="w-[100%] mt-[20px] p-[24px] bg-[#F3F4F6] rounded-xls">
      <p className="text-[20px] font-[600]">Photo Attachment</p>
      <label
        htmlFor="multi-file-upload"
        className="flex flex-col items-center mt-[20px]"
      >
        <span className="text-Black w-[100%] text-center text-[14px] h-[40px] rounded-xls p-[10px] bg-BgSoftWhite border-[1.5px] border-[#D9D9D9] focus:outline-none focus:border-Primary/20">
          Drag and Drop your files or{" "}
          <span className="text-Primary">Browse</span>
        </span>
        <input
          id="multi-file-upload"
          type="file"
          accept="image/*"
          multiple
          onChange={handleImageChange}
          className="hidden"
        />
      </label>

      {/* Display Images */}
      <div className="flex justify-start flex-wrap gap-[20px] mt-[20px]">
        {images.map((image, index) => (
          <div
            key={index}
            className="relative border rounded-md overflow-hidden"
          >
            <img
              src={image}
              alt={`Uploaded ${index}`}
              className="object-cover w-[80px] h-[80px] rounded-xls"
            />
            <button
             
              className="absolute top-1 right-1 text-Negative text-[20px]"
            >
              <TiDeleteOutline />
            </button>
          </div>
        ))}
      </div>
    </div>
  );
};

export default PhotoAttachment;
